...
flow_path = './datafiles/shutterstock/flow_pairs/'

save_path_root = './datafiles/shutterstock/sequences_select_pairs_midas/'
...
def read_flow_data(key, frame_id_1, frame_id_2):
    data = np.load(join(flow_path, key, f'flowpair_{frame_id_1:05d}_{frame_id_2:05d}.npz'))
    data_dict = {}
    for k in data.keys():
        data_dict[k] = data[k]
    return data_dict
...
def datadict_from_pair(key, pair):
    ...
    if 'motion_seg' in im1_data.keys():
        if im1_data['motion_seg'].item() is not None:
