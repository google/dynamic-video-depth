...
# from https://github.com/google/mannequinchallenge/blob/3448d9d49dc130db7ed18053b70f66bc157d238f/models/hourglass.py#L19
...
class HourglassModel(nn.Module):
    def __init__(self, num_input=3, noexp=False):
    ...
    def forward(self, input_):
        ...
        if self.noexp:
            ...
            depth = torch.exp(pred_d)

        return depth


class HourglassModel_Embed(nn.Module):
    def __init__(self, num_input=3, noexp=False, use_embedding=False, n_embedding=100):
        super(HourglassModel_Embed, self).__init__()
        self.net_depth = HourglassModel(num_input, noexp)

        self.use_embedding = use_embedding
        if use_embedding:
            self.embedding = nn.Embedding(n_embedding, 1, _weight=torch.ones([n_embedding, 1]))
    ...
    def forward(self, input_, embed_index=None):
        depth = self.net_depth(input_)
        return depth
